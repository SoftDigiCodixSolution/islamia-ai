...
const qaris = [
  ...
  { name: 'Maher Al-Muaiqly', code: 'Maher_AlMuaiqly_128kbps' },
]
...
function SurahReader({ surahNumber, translation }: Props) {
  ...
  const [playMode, setPlayMode] = useState<'single' | 'surah' | 'continuous'>('single')
  ...
  const playAyah = (ayah: Ayah) => {
    ...
    audio.play()
    audio.onended = () => {
      if (playMode === 'surah' || playMode === 'continuous') {
  ...
  return (
    <div style={{ fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{
        ...
        <p style={{ margin: '0 0 0.5rem', opacity: 0.7, fontSize: '0.85rem', letterSpacing: '2px' }}>
          {surahInfo?.revelationType?.toUpperCase()} • {surahInfo?.numberOfAyahs} AYAHS
        </p>
        <h1 style={{
          fontSize: '3rem', margin: '0 0 0.3rem',
          fontFamily: 'serif', letterSpacing: '4px'
        }}>{surahInfo?.name}</h1>
        <h2 style={{ fontSize: '1.2rem', margin: '0 0 0.3rem', fontWeight: '400', opacity: 0.9 }}>
        ...
        <div style={{ display: 'flex', gap: '0.8rem', justifyContent: 'center', flexWrap: 'wrap' }}>
          <button
            onClick={playingAyah ? stopAudio : playSurah}
            style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
          ...
          <select
            ...
            onChange={e => {
              stopAudio()
              setSelectedQari(qaris.find(q => q.code === e.target.value) || qaris[0])
            }}
            style={{
              padding: '0.4rem 0.8rem', borderRadius: '8px',
              border: '1px solid #ddd', fontSize: '0.85rem', cursor: 'pointer'
            }}
          >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          {[
            ...
            <button
              key={mode.id}
              onClick={() => { stopAudio(); setPlayMode(mode.id as any) }}
              style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
            ...
            <div>
              ...
              <p style={{ margin: 0, fontSize: '0.8rem', opacity: 0.7 }}>
                {selectedQari.name} • {playMode === 'surah' ? 'Full Surah' : 'Single Ayah'}
              </p>
            </div>
          </div>
        ...
        {ayahs.map(ayah => (
          <div
            ...
            style={{
              ...
              border: highlightedAyah === ayah.numberInSurah
                ? '2px solid #4ade80'
                : '1px solid #e8e8e8',
              transition: 'all 0.3s',
          ...
          >
            ...
            <p style={{
              ...
              direction: 'rtl', borderBottom: showArabicOnly ? 'none' : '1px solid #f0f0f0',
              paddingBottom: '1rem'
            }}>{ayah.text}</p>
